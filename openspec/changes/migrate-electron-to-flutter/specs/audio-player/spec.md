...
- Playlist store: `biu/src/store/play-list.ts`
- Progress store: `biu/src/store/play-progress.ts`
- Play modes: `biu/src/common/constants/audio.tsx`
- Audio URL helpers: `biu/src/common/utils/audio.ts`
- Playbar UI: `biu/src/layout/playbar/`
...
interface PlayData {
  ...
  type: "mv" | "audio";
  title: string;
  bvid?: string;       // Video ID
  sid?: number;        // Audio/Song ID
  cid?: string;        // Video part/page ID
  cover?: string;
  ownerName?: string;
  audioUrl?: string;
  duration?: number;
...
dependencies:
  just_audio: ^0.9.x
  just_audio_background: ^0.0.x  # iOS/Android background
  audio_session: ^0.1.x
  audio_service: ^0.18.x  # Media session integration
```
