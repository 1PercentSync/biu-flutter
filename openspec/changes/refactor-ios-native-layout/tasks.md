...
### 1.1 Theme Constants
- [ ] 1.1.1 Add iOS layout constants to `shared/theme/app_theme.dart`:
  ```dart
  ...
  ```
- [ ] 1.1.2 Update existing constants if needed (keep backward compatibility)

### 1.2 Glass Styles Utility
- [ ] 1.2.1 Create `shared/widgets/glass/glass_styles.dart`:
...
- [ ] 1.2.2 Document that colors derive from user's `backgroundColor` setting
- [ ] 1.2.3 Add helper method for ImageFilter blur creation

### 1.3 FrostedGlass Base Component
- [ ] 1.3.1 Create `shared/widgets/glass/frosted_glass.dart`:
...
- [ ] 1.3.2 Handle edge case when widget is not visible (skip blur for performance)
- [ ] 1.3.3 Export from `shared/widgets/glass/glass.dart` barrel file

## 2. Bottom Section Components

### 2.1 GlassBottomNav Component
- [ ] 2.1.1 Create `shared/widgets/glass/glass_bottom_nav.dart`:
...
- [ ] 2.1.2 Handle safe area bottom padding via `MediaQuery.of(context).padding.bottom`
- [ ] 2.1.3 Implement selection state and onTap callbacks
- [ ] 2.1.4 Ensure touch targets meet minimum 44pt iOS guideline

### 2.2 MiniPlaybar Refactor
- [ ] 2.2.1 Modify `shared/widgets/playbar/mini_playbar.dart`:
...
- [ ] 2.2.2 Update cover image size from 44px to 36px
- [ ] 2.2.3 Remove top progress bar (or relocate based on design decision)
- [ ] 2.2.4 Adjust control button layout (prev/play/next in row)
- [ ] 2.2.5 Use `primaryColorProvider` for accent colors

## 3. MainShell Layout Refactor

### 3.1 Stack-Based Layout
- [ ] 3.1.1 Modify `MainShell` in `core/router/app_router.dart`:
  ...
  - Calculate content bottom padding dynamically:
    ...
    ```
- [ ] 3.1.2 Add bottom glass backdrop as `Positioned` widget:
  - Position: `bottom: 0, left: 0, right: 0`
  - Height: `AppTheme.bottomNavHeight + safeAreaBottom`
- [ ] 3.1.3 Position MiniPlaybar as floating widget:
  - Position: `bottom: navHeight + safeBottom + margin, left: margin, right: margin`
- [ ] 3.1.4 Position GlassBottomNav at bottom
- [ ] 3.1.5 Ensure child content receives proper bottom padding

### 3.2 Content Padding Management
- [ ] 3.2.1 Create mechanism for child screens to know required bottom inset
- [ ] 3.2.2 Consider using `MediaQuery.removePadding` or custom InheritedWidget
- [ ] 3.2.3 Verify CustomScrollView/ListView in child screens scroll correctly under floating elements

## 4. Home Screen Tab Refactor

### 4.1 AdaptiveTabHeader Component
- [ ] 4.1.1 Create tab header widget (can be in home_screen.dart or separate file):
  - Accept list of tab labels
  - Accept current index and onTap callback
  - Use `LayoutBuilder` to get available width
- [ ] 4.1.2 Implement font size calculation algorithm:
...
- [ ] 4.2.1 Convert `HomeScreen` to `ConsumerStatefulWidget`
- [ ] 4.2.2 Add `PageController` for tab content
- [ ] 4.2.3 Implement three tab pages:
...
- [ ] 4.2.4 Sync tab header selection with PageView page changes
- [ ] 4.2.5 Support swipe gestures to switch tabs

### 4.3 Top Glass Backdrop
- [ ] 4.3.1 Add top `FrostedGlass` backdrop:
  - Position: `top: 0, left: 0, right: 0`
  - Height: `safeAreaTop + headerHeight` (approximately 66px header content)
- [ ] 4.3.2 Position tab header above glass backdrop (higher z-index)
- [ ] 4.3.3 Calculate content top padding to account for header

### 4.4 Content Area Padding
- [ ] 4.4.1 Each tab page content needs:
...
- [ ] 4.4.2 Ensure RefreshIndicator works with new padding structure

## 5. Supporting Updates

### 5.1 PlaybarScaffold Update
- [ ] 5.1.1 Review `PlaybarScaffold` in `app_router.dart`
- [ ] 5.1.2 Update to use floating MiniPlaybar style (if applicable to non-shell routes)
...
### 5.3 Export Updates
- [ ] 5.3.1 Update `shared/widgets/playbar/playbar.dart` exports if needed
- [ ] 5.3.2 Create `shared/widgets/glass/glass.dart` barrel file with all glass exports
...
- [ ] 7.1.1 Add dartdoc comments to all new public classes/methods
- [ ] 7.1.2 Document the glass color computation logic
- [ ] 7.1.3 Add comments explaining safe area handling
